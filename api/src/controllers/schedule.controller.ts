...
export class ScheduleController {
   ...
   */
  async getAvailableClassrooms(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { date, lessonNumber, idLesson } = req.query as { date?: string; lessonNumber?: string; idLesson?: string };

      if (!date || !this.isValidDateFormat(date)) {
        return next(createError('Invalid or missing date. Expected YYYY-MM-DD', 400));
      }
      const lessonNumberNum = Number(lessonNumber);
      if (!lessonNumber || Number.isNaN(lessonNumberNum) || lessonNumberNum <= 0) {
        return next(createError('Invalid or missing lessonNumber', 400));
      }
      ...
      const substitutionsAtTimeslot = await this.prisma.substitution.findMany({
        where: {
      ...
      const occupiedClassroomIds = new Set<number>([
        ...lessonsAtTimeslot.map((lesson: { idClassroom: number | null }) => lesson.idClassroom).filter((id): id is number => id != null),
        ...substitutionsAtTimeslot.map((substitution: { idClassroom: number }) => substitution.idClassroom),
      ]);
   ...
   * Создать замещение
   */
  async createSubstitution(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!idLesson || !idTeacher || !idClassroom) {
        return next(createError('Missing required fields: idLesson, idTeacher, idClassroom', 400));
      }
      ...
      if (error?.code === 'P2002') {
        return next(createError('Substitution already exists for this lesson and date', 409));
      }
      next(error);
    }
  }
   ...
   */
  async deleteSubstitution(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const existing = await this.prisma.substitution.findUnique({ where: { id: substitutionId } });
      if (!existing) {
        return next(createError('Substitution not found', 404));
      }

      await this.prisma.substitution.delete({ where: { id: substitutionId } });
      res.status(204).send();
    } catch (error) {
      next(error);
    }
