...
export class LessonService extends BaseService<Lesson> {
	...
	async createLesson(data: CreateLessonRequest): Promise<Lesson> {
		...
		const teacher = await this.lessonRepository[
			"prisma"
		].teacher.findUnique({
	...
	async getLessonsWithDetails(
		filters?: LessonFilters
	): Promise<LessonWithDetails[]> {
		const lessons = await this.lessonRepository.findMany(filters);

		return lessons.map((lesson) =>
			this.transformToLessonWithDetails(lesson)
		);
	}

	async getPaginatedLessons(
		filters?: LessonFilters & { page?: number; limit?: number }
	): Promise<PaginatedResult<LessonWithDetails>> {
	...
	async getTeacherSchedule(
		...
		const lessons = await this.lessonRepository.findByTeacherAndDate(
			teacherId,
			date
		);
		return lessons.map((lesson) =>
	...
	async getClassSchedule(
		...
		const lessons = await this.lessonRepository.findByClassAndDate(
			classId,
			date
		);
		return lessons.map((lesson) =>
	...
	private async checkConflicts(
		...
		const teacherConflicts = await this.lessonRepository[
			"prisma"
		].lesson.findMany({
			where: {
				...where,
		...
		const classConflicts = await this.lessonRepository[
			"prisma"
		].lesson.findMany({
			where: {
				...where,
		...
		const classroomConflicts = await this.lessonRepository[
			"prisma"
		].lesson.findMany({
			where: {
				...where,
	...
	private transformToLessonWithDetails(
		lesson: LessonWithIncludes
	): LessonWithDetails {
		const startTime = lesson.lessonSchedule.timeBegin
			.toTimeString()
			.slice(0, 5);
		const endTime = lesson.lessonSchedule.timeEnd
			.toTimeString()
			.slice(0, 5);
		...
		return {
			...
			groupNumber: (() => {
				const name = lesson.classGroup?.name;
				if (!name) return null;
