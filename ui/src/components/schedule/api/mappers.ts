...
// Lessons
export const getLessonForClass = (
	lessons: Lesson[],
	classId: number,
	day: Date,
	lessonNumber: number
	...
	const lesson = lessons.find(
		l => l.idClass === classId && l.dayOfWeek === dbDayOfWeek && l.lessonNumber === lessonNumber
	);
...
export const getLessonForTeacher = (
	lessons: Lesson[],
	teacherId: number,
	day: Date,
	lessonNumber: number
	...
	const substituted = lessons.find(
		l =>
			l.dayOfWeek === dbDayOfWeek &&
			l.lessonNumber === lessonNumber &&
			l.substitution &&
			l.substitution.idTeacher === teacherId
	);
	if (substituted) {
		const subSecondary: string[] = [];
		if (substituted.subjectName) subSecondary.push(substituted.subjectName);
		if (substituted.substitution && substituted.substitution.classroomNumber)
			subSecondary.push(`каб. ${substituted.substitution.classroomNumber}`);
		return {
			primary: substituted.className,
