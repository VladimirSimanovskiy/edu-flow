...
export const useScheduleFilters = ({
  ...
  const [filters, setFilters] = useState<ScheduleFilterState>(() => filterService.createInitialState());
  ...
  const teacherFilterOptions: ValuesFilterOptions<number> = useMemo(() => {
    const updateTeacherFilter = (inList: boolean, items: number[]) => {
      setFilters(prevFilters => {
        const newFilters = filterService.updateFilter(prevFilters, 'teachers', { inList, items });
  ...
  const classFilterOptions: ValuesFilterOptions<number> = useMemo(() => {
    const updateClassFilter = (inList: boolean, items: number[]) => {
      setFilters(prevFilters => {
        const newFilters = filterService.updateFilter(prevFilters, 'classes', { inList, items });
        onFiltersChange?.(newFilters);
        return newFilters;
      });
    };
