...
import { Button, Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, FormField, Select } from '../../ui';
...
export const CreateSubstitutionModal: React.FC<CreateSubstitutionModalProps> = ({ open, onClose, teachers, substituteTeacherId }) => {
  ...
  const substituteTeacher = useMemo(() => teachers.find(t => t.id === substituteTeacherId), [teachers, substituteTeacherId]);

  useEffect(() => {
    const fetchClassrooms = async () => {
      if (!mode) return;
      const free = await apiClient.getAvailableClassrooms(mode.date, mode.lessonNumber, mode.idOriginalLesson);
      setClassrooms(free);
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">Форма создания замещения для выбранного урока</DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <FormField label="Кабинет">
            <Select
              ...
              options={classrooms.map(c => ({ value: String(c.id), label: `каб. ${c.number}` }))}
        ...
        <DialogFooter>
          ...
          <Button onClick={onSave} disabled={saving || !selectedClassroomValue}>Сохранить</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
