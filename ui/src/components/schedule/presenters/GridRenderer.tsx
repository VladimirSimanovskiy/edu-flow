...
export const GridRenderer: React.FC<GridRendererProps> = ({ model, renderCell, renderHeader, className, enableDragScroll = true, rowHeaderLabel, rowHeaderControls }) => {
  ...
  const dayColStyle: React.CSSProperties | undefined = hasGroups ? undefined : { width: `${(100 / Math.max(model.columns.length, 1)).toFixed(3)}%` };

  return (
    <div {...containerProps}>
      <table className={[tableWidthClass, 'border-collapse', hasGroups ? 'table-fixed' : ''].join(' ')}>
        <thead>
          {hasGroups && (
            <tr className="sticky top-0 z-20 bg-white">
              <th rowSpan={2} className="sticky left-0 bg-white z-30 text-left align-middle px-2 py-2 w-40 min-w-[10rem] border-r border-gray-300 text-xs sm:text-sm font-medium text-gray-600">
              ...
              {(() => {
                const groups = model.columns.reduce<Array<{ key: string; label: string; span: number }>>((acc, col) => {
                ...
                return groups.map((g, i) => (
                  <th key={g.key + g.label} colSpan={g.span} className={["px-2 py-2 text-sm font-semibold text-gray-700 text-center border-b border-gray-200", i > 0 ? 'border-l border-gray-200' : ''].join(' ')}>
                    {g.label}
                  </th>
                ));
              })()}
          ...
          <tr className={hasGroups ? 'sticky top-10 z-20 bg-white' : 'sticky top-0 z-20 bg-white border-b border-gray-200'}>
            {!hasGroups && (
            ...
            {model.columns.map((col, idx) => {
              ...
              const groupBreak = hasGroups && (!prev || prev.groupKey !== col.groupKey);
              return (
                <th
                  key={col.key}
                  className={[
        ...
        <tbody>
          {model.rows.map((row, rowIdx) => (
            <tr key={row.id} className={[
              ...
              {model.columns.map((col, idx) => {
                const cell = model.cells.find(c => c.rowId === row.id && c.columnKey === col.key) || { rowId: row.id, columnKey: col.key, lessons: [] };
                const prev = idx > 0 ? model.columns[idx - 1] : undefined;
                const groupBreak = hasGroups && (!prev || prev.groupKey !== col.groupKey);
                return (
                  <td key={col.key} className={["px-2 py-2 align-top border-r border-gray-100 last:border-r-0", colWidthCell, groupBreak ? 'border-l border-gray-200' : ''].join(' ')} style={dayColStyle}>
                    {renderCell(cell)}
                  </td>
                );
              })}
