...
export const ClassGrid: React.FC<Props> = ({ viewType, date, lessons, teachers, classes }) => {
  ...
  const renderCellContent = (cell: { rowId: number; columnKey: string; lessons: Lesson[] }): React.ReactNode => {
    ...
    const lessonNumber = viewType === 'day'
    ...
    const lessonData: LessonData | undefined = content ? {
      ...
      secondary: [content.teacherName, `каб. ${content.classroomNumber}`].filter(Boolean) as string[],
  ...
  return (
    <GridRenderer
      ...
      rowHeaderControls={<ScheduleColumnFilter options={getClassFilterOptions(classes || [])} isActive={isClassFilterActive} />}
    />
  );
};
