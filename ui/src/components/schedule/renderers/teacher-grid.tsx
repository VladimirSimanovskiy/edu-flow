...
export const TeacherGrid: React.FC<Props> = ({ viewType, date, lessons, teachers, classes }) => {
  const { isTeacherVisible, getTeacherFilterOptions, isTeacherFilterActive } = useScheduleFiltersContext();
  const { lessonNumbers } = useLessonNumbers();
  const timeView = viewType === 'day' ? new DayViewStrategy(lessonNumbers) : new WeekViewStrategy(lessonNumbers);
  const entity = new TeacherDimensionStrategy(isTeacherVisible);
  const model = buildLessonMatrix({ lessons, date, timeView, entity, entities: { teachers, classes } });
  ...
  const { onTeacherWeekHoverChange, isTeacherWeekHoverLinked, onTeacherDayHoverChange, isTeacherDayHoverLinked } = HoverLink.useSubstitutionHover(lessons);
  const [modalOpen, setModalOpen] = useState(false);
  const [selectedSubstituteTeacherId, setSelectedSubstituteTeacherId] = useState<number | null>(null);
  const queryClient = useQueryClient();

  const renderCellContent = (cell: { rowId: number; columnKey: string; lessons: Lesson[] }): React.ReactNode => {
    ...
    const lessonNumber = viewType === 'day' ? Number(cell.columnKey) : Number(cell.columnKey.split('-')[1]);
    ...
    const getActiveSubstitutionForLesson = (l: Lesson) => {
      const single = (l as any).substitution as (undefined | { id: number; date: Date | string; idTeacher: number; classroomNumber?: number });
      if (single && sameDate(single.date, cellDate)) return single;
      const arr = (l as any).substitutions as Array<{ id: number; date: Date | string; idTeacher: number; classroomNumber?: number }> | undefined;
      return arr?.find(s => sameDate(s.date, cellDate));
    };

    const direct = lessons.find(l => l.idTeacher === teacherId && l.dayOfWeek === dayOfWeek && l.lessonNumber === lessonNumber);
    ...
    const subSourceLesson = lessons.find(l =>
    ...
    const subRecord = subSourceLesson ? getActiveSubstitutionForLesson(subSourceLesson) : undefined;
    ...
    const lessonData: LessonData | undefined = content ? {
      ...
      secondary: [content.subjectName, displayClassroomNumber ? `каб. ${displayClassroomNumber}` : undefined].filter(Boolean) as string[],
    ...
    const onClick = content && flags.highlight ? () => {
      const classId = content.idClass;
      const current = highlight.highlightedClassId;
      if (current === classId) clearHighlight(); else setHighlightedClass(classId, date);
    } : undefined;

    // Context menu actions for substitution
    const originalLessonForSlot = direct || subSourceLesson;
    const canCreateSubstitution = Boolean(flags.substitution && originalLessonForSlot && !getActiveSubstitutionForLesson(originalLessonForSlot));
    ...
    const handleCreateSubstitution = () => {
      ...
      const freeTeacherIds: number[] = (teachers || [])
        .filter(t => {
          const hasOwn = lessons.some(l => l.idTeacher === t.id && l.dayOfWeek === dayOfWeek && l.lessonNumber === lessonNumber);
          const hasSub = lessons.some(l => {
            const s = getActiveSubstitutionForLesson(l);
            return l.dayOfWeek === dayOfWeek && l.lessonNumber === lessonNumber && Boolean(s && s.idTeacher === t.id);
          });
          return !hasOwn && !hasSub;
        })
        .map(t => t.id);
      const theDate = viewType === 'day' ? date : addDays(startOfWeek(date, { weekStartsOn: 1 }), (Number(cell.columnKey.split('-')[0]) - 1));
      const dateStr = toDateKey(theDate);
      enterSubstitutionMode({ date: dateStr, lessonNumber, idOriginalLesson: originalLessonForSlot.id, idOriginalTeacher: originalLessonForSlot.idTeacher, classId: originalLessonForSlot.idClass }, freeTeacherIds);
    };
    ...
    const onMouseEnter = content && flags.hoverLink ? () => {
      const payload = viewType === 'week'
    ...
    const onMouseLeave = content && flags.hoverLink ? () => {
      const payload = viewType === 'week'
        ? onTeacherWeekHoverChange(teacherId, date, lessonNumber, false)
        : onTeacherDayHoverChange(teacherId, dayOfWeek, lessonNumber, false);
      setHoverLinked(payload);
    } : undefined;
    ...
    const isHoverLinked = () => {
      ...
      return viewType === 'week'
        ...
        : isTeacherDayHoverLinked(teacherId, dayOfWeek, lessonNumber, highlight.hoverLinked);
    };
    ...
    const isEmptySlotHighlighted = (() => {
      ...
      const isSameSlot = mode.lessonNumber === lessonNumber && (toDateKey(mode.date) === toDateKey(cellDate));
      if (!isSameSlot) return false;
      // Проверяем доступность учителя
      if (highlight.freeTeacherIdsAtTimeslot && !highlight.freeTeacherIdsAtTimeslot.includes(teacherId)) return false;
      // Fallback вычисление, если freeTeacherIdsAtTimeslot отсутствует
      const hasOwn = lessons.some(l => l.idTeacher === teacherId && l.dayOfWeek === dayOfWeek && l.lessonNumber === lessonNumber);
      const hasSub = lessons.some(l => {
    ...
    const baseCell = (
      <LessonCell
        ...
        onEmptyClick={flags.substitution ? () => {
          const mode = highlight.substitutionMode;
        ...
        <ContextMenu>
          ...
          <ContextMenuContent>
            {canCreateSubstitution && (
              <ContextMenuItem onSelect={handleCreateSubstitution}>Создать замещение</ContextMenuItem>
            )}
            {canDeleteSubstitution && (
              <ContextMenuItem onSelect={handleDeleteSubstitution}>Удалить замещение</ContextMenuItem>
            )}
          </ContextMenuContent>
        </ContextMenu>
      );
  ...
  return (
    <>
      <GridRenderer
        ...
        rowHeaderControls={<ScheduleColumnFilter options={getTeacherFilterOptions(teachers || [])} isActive={isTeacherFilterActive} />}
      />
      {flags.substitution && (
