    ...
    >
      {lessonNumbers?.map(lessonNumber => {
        ...
        const isHighlighted = lesson
          ? (isLessonHighlighted ? isLessonHighlighted(lessonNumber, lesson) : false)
          : (isEmptyCellHighlighted ? isEmptyCellHighlighted(lessonNumber) : false);
        ...
        const cell = (
          <LessonCell
            ...
            onEmptyClick={onEmptyCellClick ? () => onEmptyCellClick(lessonNumber) : undefined}
          />
        );
          ...
          <ContextMenu key={lessonNumber}>
            ...
            <ContextMenuContent>
              {canCreateSubstitution && onLessonContextMenu ? (
                <ContextMenuItem onSelect={() => onLessonContextMenu(lessonNumber, lesson)}>
                  Создать замещение
                </ContextMenuItem>
              ) : null}
              {canDeleteSubstitution && onLessonDeleteSubstitution ? (
                <ContextMenuItem onSelect={() => onLessonDeleteSubstitution(lessonNumber, lesson)}>
                  Удалить замещение
                </ContextMenuItem>
              ) : null}
            </ContextMenuContent>
