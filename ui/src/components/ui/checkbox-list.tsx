    ...
    >
      <div className="space-y-2">
        ...
        <div className="flex items-center space-x-2 p-2 border-b">
          <Checkbox
            id="select-all"
            checked={selectAllState === 'checked' ? true : selectAllState === 'indeterminate' ? 'indeterminate' : false}
        ...
        <div className="space-y-1">
          {items.map((item, index) => (
            <div key={`${item}-${index}`} className="flex items-center space-x-2 p-2 hover:bg-gray-50 rounded">
              <Checkbox
                id={`item-${item}`}
