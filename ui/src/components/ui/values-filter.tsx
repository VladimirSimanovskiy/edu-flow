...
export const ValuesFilter = <T extends ItemValue>({
  options,
  className,
}: ValuesFilterProps<T>) => {
  ...
  const [lastTriggeredState, setLastTriggeredState] = useState<{allChecked: boolean, toggledItems: T[]} | null>(null);
  ...
  const triggerFilterChange = useCallback((inList: boolean, items: T[]) => {
    if (filterTimer) {
  ...
  }, [state.allChecked, state.toggledItems, triggerFilterChange, isInitializing, lastTriggeredState]);
